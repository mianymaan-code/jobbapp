...
["sökord1", "sökord2", "sökord3"]"""
    ...
    try:
        ...
        return {"fel": f"Fel vid API-anrop: {e}"}
def analysera_jobb_från_anteckningar(jobb):
    """Analyserar ett jobb baserat på anteckningar istället för jobbeskrivning."""
    api_key = os.environ.get("ANTHROPIC_API_KEY")
    if not api_key:
        return {"fel": "ANTHROPIC_API_KEY saknas."}
    ...
    anteckningar = jobb.get("anteckningar", "").strip()
    if not anteckningar:
        return {"fel": "Inga anteckningar att analysera. Klistra in jobbannonsen under Anteckningar först."}
